    ...
    if (yaOcupada) {
      ...
      if (existente) {
        retirar(
          ...
          null,
          0,
          `Removed ${existente.crop_id} from bed ${cama + 1} (${x}, ${y})`,
        );
      }
      return;
    }
      ...
      <div className="jardin-layout">
        <div className="jardin-principal">
          ...
          <div className="selector-cultivo tarjeta">
            ...
            <span className="pista-selector">
              Click a cell to plant, click again to remove
            </span>
          </div>
          ...
          <div className="camas">
            {Array.from({ length: NUMERO_CAMAS }, (_, cama) => (
              <section
                key={cama}
                className="cama tarjeta"
                aria-label={`Bed ${cama + 1}`}
              >
                <h2 className="titulo-cama">Bed {cama + 1}</h2>
                <div
                  className="grid"
                  role="grid"
                  aria-label={`Bed ${cama + 1} grid`}
                >
                  {Array.from({ length: ALTO_GRID }, (_, y) => (
                    <div key={y} className="fila-grid" role="row">
                      {Array.from({ length: ANCHO_GRID }, (_, x) => {
                        const ocupante = beds[cama]?.find(
                          (c) => c.x === x && c.y === y,
                        );
                        const cultivo = ocupante
                          ? CULTIVOS.find((c) => c.id === ocupante.crop_id)
                          : undefined;
                        return (
                          <button
                            key={x}
                            ...
                            role="gridcell"
                            className={
                              ocupante ? 'celda ocupada' : 'celda vacia'
                            }
                            ...
                            title={
                              ocupante
                                ? `Remove ${cultivo?.name ?? ocupante.crop_id}`
                                : `Plant ${cropSeleccionado}`
                            }
                          >
